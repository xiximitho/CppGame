...
void carve_lake(TileMap& map, Rng& rng, int z) {
    ...
    for (int y = cy - radius; y <= cy + radius; ++y) {
        for (int x = cx - radius; x <= cx + radius; ++x) {
            ...
            map.set_object(pos, kTileEmpty, true);
        }
    }
}

void carve_path(TileMap& map, Rng& rng, int z) {
    ...
    while (y < map.height()) {
        ...
        if (map.in_bounds(pos)) {
            ...
            map.set_object(pos, kTileEmpty, false);
        }
        if (rng.chance(0.35F)) {
...
void build_room(TileMap& map, Rng& rng, int z) {
    ...
    for (int y = y0; y < y0 + h; ++y) {
        for (int x = x0; x < x0 + w; ++x) {
            ...
            map.set_object(pos, is_door ? kTileEmpty : tiles::kWall, !is_door);
        }
    }
}
...
void scatter_trees(TileMap& map, Rng& rng, int z, int count) {
    for (int i = 0; i < count; ++i) {
        ...
        if (map.at(pos).ground == tiles::kGrass &&
            ...
            map.set_object(pos, tiles::kTree, true);
        }
    }
}

void wall_border(TileMap& map, int z) {
    for (int x = 0; x < map.width(); ++x) {
        for (const int y : {0, map.height() - 1}) {
            const TilePos pos{static_cast<std::int16_t>(x),
                              static_cast<std::int16_t>(y),
                              static_cast<std::int8_t>(z)};
            map.set_object(pos, tiles::kWall, true);
        }
    }
    for (int y = 0; y < map.height(); ++y) {
...
void build_upper_platform(TileMap& map, Rng& rng) {
    ...
    for (int y = y0; y < y0 + h; ++y) {
        for (int x = x0; x < x0 + w; ++x) {
            ...
            const bool on_edge =
                x == x0 || x == x0 + w - 1 || y == y0 || y == y0 + h - 1;
            map.set_object(pos, on_edge ? tiles::kWall : kTileEmpty, on_edge);
        }
    }
}
...
TileMap generate_demo_map(const MapGenSettings& settings) {
    ...
    for (int i = 0; i < 3; ++i) {
        carve_lake(map, rng, 0);
    }
    for (int i = 0; i < 5; ++i) {
        build_room(map, rng, 0);
    }
    for (int i = 0; i < 3; ++i) {
        carve_path(map, rng, 0);
    }
    scatter_trees(map, rng, 0, (settings.width * settings.height) / 60);
    wall_border(map, 0);

    build_upper_platform(map, rng);
